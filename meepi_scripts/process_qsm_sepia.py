...
def _get_wk_outputs(unwrap_dir: str, out_prefix: str) -> tuple[list[str], list[str]]:
    """Find unwrapped phase files and mask files generated by wk-unwrap-phase."""
    ...
    mask_files = sorted([path for path in nifti_files if 'mask' in os.path.basename(path).lower()])
    phase_files = _sort_wk_outputs(
        [path for path in nifti_files if 'mask' not in os.path.basename(path).lower()]
    )
    return phase_files, mask_files
...
def unwrap_phase_data(
    ...
    existing_phase_files, existing_mask_files = _get_wk_outputs(unwrap_dir, out_prefix)
    if (
        len(existing_phase_files) == len(run_data['bold_phase'])
        and len(existing_mask_files) == 1
        and not overwrite
    ):
        return existing_phase_files, existing_mask_files[0]
    ...
    unwrapped_phase_files, mask_files = _get_wk_outputs(unwrap_dir, out_prefix)
    ...
    if len(mask_files) != 1:
        raise FileNotFoundError(
            'wk-unwrap-phase did not produce exactly one automask NIfTI. '
            f'Found {len(mask_files)}: {mask_files}'
        )
    return unwrapped_phase_files, mask_files[0]
...
def _load_mask_volume(img: nb.spatialimages.SpatialImage, volume_idx: int, n_volumes: int) -> np.ndarray:
    if img.ndim == 3:
        return np.asanyarray(img.dataobj)
    if img.ndim != 4:
        raise ValueError(f'wk-unwrap-phase mask must be 3D or 4D. Got shape {img.shape}')
    if img.shape[3] == n_volumes:
        return np.asanyarray(img.dataobj[..., volume_idx])
    if img.shape[3] == 1:
        return np.asanyarray(img.dataobj[..., 0])
    raise ValueError(
        'wk-unwrap-phase mask frame count does not match the BOLD data. '
        f'Mask shape {img.shape}, expected {n_volumes} frames.'
    )
...
def write_volume_inputs(
    ...
    if mask_img.shape[:3] != shapes[0][:3]:
        raise ValueError(
            'wk-unwrap-phase mask spatial shape does not match the BOLD data. '
            f'Mask shape {mask_img.shape}, BOLD shape {shapes[0]}.'
        )
    volume_inputs = []
    for volume_idx in range(n_volumes):
        ...
        mask_data = _load_mask_volume(mask_img, volume_idx, n_volumes)
        ...
        volume_mask_file = os.path.join(input_dir, f'{volume_label}_desc-wkunwrap_mask.nii.gz')
        ...
        nb.Nifti1Image(mask_data, ref_img.affine, ref_img.header).to_filename(volume_mask_file)
...
def write_matlab_scripts(
    ...
    for volume_input in volume_inputs:
        ...
        modified_sepia_script = (
            ...
            .replace('{{ mask_file_literal }}', _matlab_literal_path(str(volume_input['mask_file'])))
...
def process_run(
    ...
    unwrapped_phase_files, mask_file = unwrap_phase_data(
        run_data=run_data,
        temp_dir=temp_dir,
